...
export const CategoricalSplit = () => {
      ...
      try {
        ...
        const processed = (txRes.data.transactions || []).map((tx) => {
          ...
          return {
            ...
            // Keep the actual category from MongoDB.
            // getCategorySpending() will use inferCategory()
  ...
  /*
   * getCategorySpending:
   * - considers only outgoing transactions
   * - uses saved category when available
   * - falls back to inferCategory() for old transactions
   */
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-3 mt-6">
            {data.map((entry, index) => (
              ...
              >
                ...
                <span
                  className="text-xs text-gray-600 dark:text-gray-300 truncate w-full"
                  title={entry.name}
                >
                  {entry.name}
                </span>
